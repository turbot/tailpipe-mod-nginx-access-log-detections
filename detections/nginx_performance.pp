...
detection "nginx_cache_performance" {
  ...
  description     = "Analyze cache hit/miss ratio and identify potential cache optimization opportunities."
  severity        = "medium"
  display_columns = ["endpoint", "total_requests", "cache_hit_ratio", "cache_miss_count", "avg_response_time"]
  
  query = query.nginx_cache_performance
...
query "nginx_cache_performance" {
  sql = <<-EOQ
    with cache_stats as (
      select
        request_uri as endpoint,
        count(*) as total_requests,
        count(*) filter (where upstream_cache_status = 'HIT') as cache_hits,
        count(*) filter (where upstream_cache_status = 'MISS') as cache_miss_count,
        avg(request_time) as avg_response_time
      from
        nginx_access_log
      where
        upstream_cache_status is not null
      group by
        request_uri
      having
    ...
    select
      endpoint,
      total_requests,
      round((cache_hits::float / total_requests * 100)::numeric, 2) as cache_hit_ratio,
      cache_miss_count,
      round(avg_response_time::numeric, 3) as avg_response_time
    from
      cache_stats
    where
      (cache_hits::float / total_requests) < 0.5  -- Less than 50% cache hit ratio
    order by
      total_requests desc;
  EOQ
} 